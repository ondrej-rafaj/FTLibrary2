...
@property (nonatomic, copy)	  FT2CoreTextStyle	*style;
...
@implementation FT2CoreTextView
...
CTFontRef CTFontCreateFromUIFont(UIFont *font)
{
    CTFontRef ctFont = CTFontCreateWithName((__bridge_retained CFStringRef)font.fontName, 
                                            font.pointSize, 
                                            NULL);
    return ctFont;
}
...
- (NSDictionary *)dataForPoint:(CGPoint)point
{
    ...
    NSArray *lines = (__bridge_transfer NSArray *)CTFrameGetLines(ctframe);
    ...
    if (lineCount != 0) {
		...
		for (int i = 0; i < lineCount; i++) {
			...
			CTLineRef line = (__bridge_retained CTLineRef)[lines objectAtIndex:i];
			...
			if (CGRectContainsPoint(lineFrame, point)) {
                ...
                for(CFIndex j = 0; j < CFArrayGetCount(runs); j++) {
                    ...
                    NSDictionary* attributes = (__bridge_transfer NSDictionary*)CTRunGetAttributes(run);
...
- (void)updateFramesetterIfNeeded
{
    if (!_coreTextViewFlags.updatedAttrString) {
		...
		_framesetter = CTFramesetterCreateWithAttributedString((__bridge_retained CFAttributedStringRef)self.attributedString);
...
- (CGSize)suggestedSizeConstrainedToSize:(CGSize)size
{
	...
	if ([self isSystemUnder3_2]) {
		...
		FT2CoreTextStyle *style = [_styles objectForKey:[self defaultTagNameForKey:FTCoreTextTagDefault]];
...
{    
	...
	if (systemUnder3_2) {
		...
		for (FT2CoreTextStyle *style in _styles.allValues) {
	...
	while (!finished) {
        ...
        FT2CoreTextStyle *style = [_styles objectForKey:tagName];
        ...
        switch (tagType) {
            case FTCoreTextTagTypeOpen:
            {
                ...
                if ([tagName isEqualToString:[self defaultTagNameForKey:FTCoreTextTagLink]]) {
                    ...
                    //bullet styling
                    FT2CoreTextStyle *bulletStyle = [FT2CoreTextStyle new];
            ...
            case FTCoreTextTagTypeClose:
            {
                ...
                if (style.paragraphInset.top > 0) {
                    if (![style.name isEqualToString:[self defaultTagNameForKey:FTCoreTextTagBullet]] ||  [[currentSupernode previousNode].style.name isEqualToString:[self defaultTagNameForKey:FTCoreTextTagBullet]]) {
                        ...
                        FT2CoreTextStyle *topSpacingStyle = [[FT2CoreTextStyle alloc] init];
...
{
    FT2CoreTextView *instance = [[FT2CoreTextView alloc] initWithFrame:CGRectZero];
...
- (void)addStyle:(FT2CoreTextStyle *)style
{
...
{
	for (FT2CoreTextStyle *style in styles) {
		[_styles setValue:style forKey:style.name];
	}
	[self didMakeChanges];
    if ([self superview]) [self setNeedsDisplay];
...
- (void)applyStyle:(FT2CoreTextStyle *)style inRange:(NSRange)styleRange onString:(NSMutableAttributedString **)attributedString
{
	...
	[*attributedString addAttribute:(id)kCTFontAttributeName
							  value:(__bridge_transfer id)ctFont
							  range:styleRange];
	CFRelease(ctFont);
	...
	if (applyParagraphStyling) {
		...
		[*attributedString addAttribute:(id)kCTParagraphStyleAttributeName
								  value:(__bridge_transfer id)paragraphStyle 
								  range:styleRange];
		CFRelease(tabStops);
...
- (void)doInit
{
	...
	FT2CoreTextStyle *defaultStyle = [FT2CoreTextStyle styleWithName:FTCoreTextTagDefault];
	[self addStyle:defaultStyle];	
	
	FT2CoreTextStyle *linksStyle = [defaultStyle copy];
...
- (void)drawRect:(CGRect)rect
{
	...
	if ([self isSystemUnder3_2]) {
		...
		FT2CoreTextStyle *defaultStyle = [_styles objectForKey:[self defaultTagNameForKey:FTCoreTextTagDefault]];
